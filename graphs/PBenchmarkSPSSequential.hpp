 ...
 */
#ifndef _PERSISTENT_BENCHMARK_SPS_SEQUENTIAL_H_
#define _PERSISTENT_BENCHMARK_SPS_SEQUENTIAL_H_
 ...
 */
class PBenchmarkSPSSequential {

private:
    int numThreads;

public:
    PBenchmarkSPSSequential(int numThreads) {
    ...
    uint64_t SPSInteger(std::string& className, const seconds testLengthSeconds, const long numSwapsPerTx, const int numRuns) {
        ...
        PTM::updateTxSeq([larraySize] () {
            //PTM::pfree( (PERSIST<uint64_t>*)get_object(0) ); // TODO: re-enable this after we add the clear of objects as a transaction in CX
