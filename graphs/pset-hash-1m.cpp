...
#include <fstream>
#include <cstring>
#include "pdatastructures/TMHashMapWF.hpp"
...
#ifndef DATA_FILE
#define DATA_FILE "data/pset-hash-1m-" PTM_FILEXT ".txt"
#endif
...
#ifdef USE_CXPTM
            results[it][ir] = bench.benchmark<TMHashMapWF<uint64_t,uint64_t,cx::CX,cx::persist>,                               cx::CX>                  (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_CXREDO
            results[it][ir] = bench.benchmark<TMHashMapWF<uint64_t,uint64_t,cxredo::CXRedo,cxredo::persist>,                   cxredo::CXRedo>          (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_CXREDOTIMED
            results[it][ir] = bench.benchmark<TMHashMapWF<uint64_t,uint64_t,cxredotimed::CXRedoTimed,cxredotimed::persist>,    cxredotimed::CXRedoTimed>(cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_ROMLR
            results[it][ir] = bench.benchmark<TMHashMap<uint64_t,uint64_t,romuluslr::RomulusLR,romuluslr::persist>,     romuluslr::RomulusLR>    (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_ROMLOG
...
#else
            results[it][ir] = bench.benchmark<TMHashMap<uint64_t,uint64_t,PTM_CLASS,PTM_TYPE>, PTM_CLASS>(cName, ratio, testLength, numRuns, numKeys, nThreads);
#endif
        }
    }
