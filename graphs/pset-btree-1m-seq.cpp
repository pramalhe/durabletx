...
#include <fstream>
#include <cstring>
#include "pdatastructures/psequential/PSeqBTree.hpp"
#include "ptms/ptm.h"

#ifdef USE_ROM_LOG_FC
#include "ptms/romuluslog/RomLogFC.hpp"
#define DATA_FILE "data/pset-btree-1m-seq-romlogfc.txt"
#elif defined USE_ROM_LOG_2F_FC
#include "ptms/romuluslog/RomLog2FFC.hpp"
#define DATA_FILE "data/pset-btree-1m-seq-romlog2ffc.txt"
#elif defined USE_ROM_LOG_TS_FC
#include "ptms/romuluslog/RomLogTSFC.hpp"
#define DATA_FILE "data/pset-btree-1m-seq-romlogtsfc.txt"
#endif
#include "PBenchmarkSets.hpp"

#ifndef DATA_FILE
#define DATA_FILE "data/pset-btree-1m-seq-" PTM_FILEXT ".txt"
#endif

int main(int argc, char* argv[]) {
    const std::string dataFilename { DATA_FILE };
    vector<int> threadList = { 1 };                              // For the laptop or AWS c5.9xlarge
    ...
    for (unsigned ir = 0; ir < ratioList.size(); ir++) {
        ...
        for (unsigned it = 0; it < threadList.size(); it++) {
            ...
            ic++;
#else
            results[ic][ir] = bench.benchmark<PSeqBTree<uint64_t,PTM_CLASS,PTM_TYPE>, PTM_CLASS>(cNames[ic], ratio, testLength, numRuns, numKeys);
            ic++;
#endif
            maxClass = ic;
        }
