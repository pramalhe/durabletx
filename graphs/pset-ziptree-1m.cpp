...
#include <fstream>
#include <cstring>
#include "pdatastructures/TMZipTreeSetWF.hpp"
...
#define DATA_FILE "data/pset-ziptree-1m-cxptm.txt"
#elif defined USE_CXREDO
#include "ptms/cxredo/CXRedo.hpp"
#define DATA_FILE "data/pset-ziptree-1m-cxredo.txt"
#elif defined USE_CXREDOTIMED
#include "ptms/cxredotimed/CXRedoTimed.hpp"
#define DATA_FILE "data/pset-ziptree-1m-cxredotimed.txt"
#elif defined USE_ROMLR
#include "ptms/romuluslr/RomulusLR.hpp"
#define DATA_FILE "data/pset-ziptree-1m-romlr.txt"
#elif defined USE_ROMLOG
#include "ptms/romuluslog/RomulusLog.hpp"
#define DATA_FILE "data/pset-ziptree-1m-romlog.txt"
...
#ifndef DATA_FILE
#define DATA_FILE "data/pset-ziptree-1m-" PTM_FILEXT ".txt"
#endif
...
int main(int argc, char* argv[]) {
    ...
    for (unsigned ir = 0; ir < ratioList.size(); ir++) {
        ...
        for (unsigned it = 0; it < threadList.size(); it++) {
            ...
            std::cout << "\n----- Persistent Sets (Zip Tree)   numKeys=" << numKeys << "   ratio=" << ratio/10. << "%   threads=" << nThreads << "   runs=" << numRuns << "   length=" << testLength.count() << "s -----\n";
#ifdef USE_CXPTM
            results[it][ir] = bench.benchmark<TMZipTreeSetWF<uint64_t,cx::CX,cx::persist>,                               cx::CX>                  (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_CXREDO
...
#elif defined USE_ROMLR
            results[it][ir] = bench.benchmark<TMZipTreeSet<uint64_t,romuluslr::RomulusLR,romuluslr::persist>,     romuluslr::RomulusLR>    (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_ROMLOG
            results[it][ir] = bench.benchmark<TMZipTreeSet<uint64_t,romuluslog::RomulusLog,romuluslog::persist>,  romuluslog::RomulusLog>  (cName, ratio, testLength, numRuns, numKeys, nThreads);
#elif defined USE_ROM_LOG_FC
            results[it][ir] = bench.benchmark<TMZipTreeSet<uint64_t,romlogfc::RomLog,romlogfc::persist>,          romlogfc::RomLog>        (cName, ratio, testLength, numRuns, numKeys, nThreads);
#else
            results[it][ir] = bench.benchmark<TMZipTreeSet<uint64_t,PTM_CLASS,PTM_TYPE>, PTM_CLASS>(cName, ratio, testLength, numRuns, numKeys, nThreads);
#endif
        }
    }
