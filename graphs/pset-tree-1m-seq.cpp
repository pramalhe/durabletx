...
#include <fstream>
#include <cstring>
#include "pdatastructures/psequential/PSeqRedBlackTree.hpp"
#include "PBenchmarkSets.hpp"
#include "ptms/ptm.h"

#ifdef USE_ROM_LOG_FC
#include "ptms/romuluslog/RomLogFC.hpp"
#define DATA_FILE "data/pset-tree-1m-seq-romlogfc.txt"
#elif defined USE_ROM_LOG_2F_FC
#include "ptms/romuluslog/RomLog2FFC.hpp"
#define DATA_FILE "data/pset-tree-1m-seq-romlog2ffc.txt"
#elif defined USE_ROM_LOG_TS_FC
#include "ptms/romuluslog/RomLogTSFC.hpp"
#define DATA_FILE "data/pset-tree-1m-seq-romlogtsfc.txt"
#endif

#ifndef DATA_FILE
#define DATA_FILE "data/pset-tree-1m-seq-" PTM_FILEXT ".txt"
#endif
...
int main(int argc, char* argv[]) {
    ...
    for (unsigned ir = 0; ir < ratioList.size(); ir++) {
        ...
        for (unsigned it = 0; it < threadList.size(); it++) {
            ...
            int ic = 0;
            std::cout << "\n----- Persistent Sets (Tree)   numKeys=" << numKeys << "   ratio=" << ratio/10. << "%   threads=" << nThreads << "   runs=" << numRuns << "   length=" << testLength.count() << "s -----\n";
#ifdef USE_ROM_LOG_FC
            results[ic][ir] = bench.benchmark<PSeqRedBlackTree<uint64_t,uint64_t,romlogfc::RomLog,romlogfc::persist>,         romlogfc::RomLog>        (cNames[ic], ratio, testLength, numRuns, numKeys);
            ic++;
#elif defined USE_ROM_LOG_2F_FC
            results[ic][ir] = bench.benchmark<PSeqRedBlackTree<uint64_t,uint64_t,romlog2ffc::RomLog,romlog2ffc::persist>,     romlog2ffc::RomLog>      (cNames[ic], ratio, testLength, numRuns, numKeys);
            ic++;
#elif defined USE_ROM_LOG_TS_FC
            results[ic][ir] = bench.benchmark<PSeqRedBlackTree<uint64_t,uint64_t,romlogtsfc::RomLog,romlogtsfc::persist>,     romlogtsfc::RomLog>      (cNames[ic], ratio, testLength, numRuns, numKeys);
            ic++;
#else
            results[ic][ir] = bench.benchmark<PSeqRedBlackTree<uint64_t,uint64_t,PTM_CLASS,PTM_TYPE>, PTM_CLASS>(cNames[ic], ratio, testLength, numRuns, numKeys);
            ic++;
#endif
            maxClass = ic;
        }
