 ...
 */
#ifndef _UNDO_LOG_SINGLE_THREADED_H_
#define _UNDO_LOG_SINGLE_THREADED_H_
...
/*
 * <h1> Undo Log </h1>
 * This is a pure undo log technique. Kind of like Vista or libpmemobj (PMDK)
 * but using Flat Combining for the concurrent transactions
 */
...
#ifndef PM_FILE_NAME
#define PM_FILE_NAME   "/dev/shm/undologfc_shared"
#endif
...
struct PMetadata {
    static const uint64_t   MAGIC_ID = 0x1337babc;
    persist<void*>          rootPtrs[MAX_ROOT_POINTERS];
    uint64_t                id {0};
    volatile uint64_t       numEntries {0};
...
public:
    struct tmbase : public undologfc::tmbase { };
    ...
    static std::string className() { return "UndoLog-FC"; }
    ...
    void mapPersistentRegion(const char* filename, uint8_t* regionAddr, const uint64_t regionSize) {
        ...
        if (sizeof(PMetadata) > regionSize/2) {
            ...
            printf("Please reduce some of the settings in UndoLogFC.hpp and try again\n");
            assert(false);
        }
...
template<typename T> inline void persist<T>::pstore(T newVal) {
    ...
    if (tl_nested_write_trans != 0 && valaddr >= (uint8_t*)PM_REGION_BEGIN && valaddr < PREGION_END) {
        uint64_t idx = pmd->numEntries;
        assert (idx != TX_MAX_STORES);
        pmd->pwset[idx].addr = this;
        pmd->pwset[idx].value = val;
        PWB(&pmd->pwset[idx]);
        PFENCE();
        pmd->numEntries = idx+1;
        PWB(&pmd->numEntries);
        PFENCE();
        val = (uint64_t)newVal;
        PWB(this);
