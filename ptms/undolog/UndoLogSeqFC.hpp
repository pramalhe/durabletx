 ...
 */
#ifndef _UNDO_LOG_SEQ_FLAT_COMBINING_H_
#define _UNDO_LOG_SEQ_FLAT_COMBINING_H_
...
/*
 * <h1> Undo Log </h1>
 * This is the same as Undo Log but uses a sequence in each log entry and
 * because of that, it needs one pfence per store instead of two pfences per store.
 * Still suckz though!
 */
...
#ifndef PM_FILE_NAME
#define PM_FILE_NAME   "/dev/shm/undologseqfc_shared"
#endif

// Maximum number of registered threads that can execute transactions
static const int REGISTRY_MAX_THREADS = 128;
// Maximum number of stores in the write-set per transaction
static const uint64_t TX_MAX_STORES = 16*1024*1024ULL;
...
template<typename T> struct persist {
    ...
    // There is no load interposing in undo log
...
struct PWriteSetEntry {
    ...
    volatile uint64_t  committed;  // Only the 'committed' of the first entry is used
};
...
struct PMetadata {
    static const uint64_t   MAGIC_ID = 0x1337babd;
    persist<void*>          rootPtrs[MAX_ROOT_POINTERS];
    PWriteSetEntry          pwset[TX_MAX_STORES];
    uint64_t                id {0};
...
public:
    ...
    uint64_t                               v_size {0};   // Accessd from the store interposing method
    ...
    static std::string className() { return "UndoLog-Seq-FC"; }
    ...
    void mapPersistentRegion(const char* filename, uint8_t* regionAddr, const uint64_t regionSize) {
        ...
        if (sizeof(PMetadata) > regionSize/2) {
            ...
            printf("Please reduce some of the settings in UndoLogSeq.hpp and try again\n");
            assert(false);
        }
        ...
        if (reuseRegion) {
            ...
            pmd->pwset[0].committed = 1;
            PWB(&pmd->pwset[0].committed);
            PFENCE();
    ...
    inline void beginTx() {
        ...
        pmd->pwset[0].committed = 0;   // Ordered store (with the other values in the first log entry)
    }
    ...
    inline void endTx() {
        ...
        PFENCE();
        pmd->pwset[0].seq = 0;        // Ordered store
        pmd->pwset[0].committed = 1;  // Ordered store
        PWB(&pmd->pwset[0]);
        PSYNC();
        v_size = 0;
        v_seq++;
    ...
    /* If the committed flag is at 1, then there is nothing to recover.
     * Otherwise, find the first log entry whose sequence is different (higher)
     * than the sequence on the first log entry and revert everything up to it.
     */
    void recover() {
        if (pmd->pwset[0].seq == 0 || pmd->pwset[0].committed == 1) return;
        // Compute the size of the log
        long size = 1;
        uint64_t lseq = pmd->pwset[0].seq;
        for (; size < TX_MAX_STORES; size++) {
            if (pmd->pwset[size].seq != lseq) break;
        }
        // Restore the original values saved in the undo log, in the reverse order
        for (long i = size-1; i >= 0 ; i--) {
            PWriteSetEntry* e = &pmd->pwset[i];
            *(uint64_t*)e->addr = e->value;
            PWB(e->addr);
        }
        PFENCE();
        pmd->pwset[0].committed = 1;
    ...
    template<typename F> static void updateTx(F&& func) { gUndoLog.ns_write_transaction(func); }
    template<typename F> static void readTx(F&& func) { gUndoLog.ns_read_transaction(func); }
    // Sequential durable transactions
    template<typename F> static void updateTxSeq(F&& func) { gUndoLog.beginTx(); func(); gUndoLog.endTx(); }
    ...
    inline static R readTx(F&& func) {
        gUndoLog.ns_read_transaction([&]() {func();});
        return R{};
    }
    template<typename R,class F>
    inline static R updateTx(F&& func) {
        gUndoLog.ns_write_transaction([&]() {func();});
        return R{};
    }
    ...
    template <typename T, typename... Args> static T* tmNew(Args&&... args) {
        ...
        T* ptr = (T*)gUndoLog.esloco.malloc(sizeof(T));
    ...
    static void* tmMalloc(size_t size) {
        ...
        void* obj = gUndoLog.esloco.malloc(size);
        return obj;
    }
    ...
    static void* pmalloc(size_t size) {
        ...
        return gUndoLog.esloco.malloc(size);
    }
...
template<typename T> inline void persist<T>::pstore(T newVal) {
    ...
    if (tl_nested_write_trans != 0 && valaddr >= (uint8_t*)PM_REGION_BEGIN && valaddr < PREGION_END) {
        const uint64_t idx = gUndoLog.v_size;
        assert (idx != TX_MAX_STORES);
        pmd->pwset[idx].addr = this;           // Ordered store
        pmd->pwset[idx].value = val;           // Ordered store
        pmd->pwset[idx].seq = gUndoLog.v_seq;  // Ordered store
        PWB(&pmd->pwset[idx]);
        PFENCE();
        val = (uint64_t)newVal;
        PWB(this);
...
//
template<typename R, typename F> static R updateTx(F&& func) { return gUndoLog.updateTx<R>(func); }
template<typename R, typename F> static R readTx(F&& func) { return gUndoLog.readTx<R>(func); }
template<typename F> static void updateTx(F&& func) { gUndoLog.updateTx(func); }
template<typename F> static void readTx(F&& func) { gUndoLog.readTx(func); }
template<typename F> static void updateTxSeq(F&& func) { gUndoLog.beginTx(); func(); gUndoLog.endTx(); }
template<typename F> static void readTxSeq(F&& func) { func(); }
template<typename T, typename... Args> T* tmNew(Args&&... args) { return UndoLog::tmNew<T>(std::forward<Args>(args)...); }
template<typename T> void tmDelete(T* obj) { UndoLog::tmDelete<T>(obj); }
inline static void* get_object(int idx) { return UndoLog::get_object(idx); }
inline static void put_object(int idx, void* obj) { UndoLog::put_object(idx, obj); }
inline static void* tmMalloc(size_t size) { return UndoLog::tmMalloc(size); }
inline static void tmFree(void* obj) { UndoLog::tmFree(obj); }
...
#endif /* _UNDO_LOG_SINGLE_THREADED_H_ */
