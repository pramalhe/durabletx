 ...
 */
#ifndef _REDO_LOG_FLAT_COMBINING_H_
#define _REDO_LOG_FLAT_COMBINING_H_
...
#ifndef PM_FILE_NAME
#define PM_FILE_NAME   "/dev/shm/redologfc_shared"
#endif
...
struct PWriteSetEntry {
    volatile void*    addr;  // Address of value to change
    volatile uint64_t val;   // Desired value to change to
};


// The persistent write-set (redo log)
struct PWriteSet {
...
struct WriteSet {
    ...
    inline void persistAndFlushLog(PWriteSet* const pwset) {
        ...
        // Flush the log
        flushFromTo(&pwset->plog[0], &pwset->plog[numStores+1]);
        PFENCE();                           // 1st persistent fence
        // Make the log consistent (by writting the number of entries in the log)
        pwset->numStores = numStores;
        PWB(&pwset->numStores);
        PSYNC();                            // 2nd persistent fence : durable commit
        // Apply modifications and flush them
        for (uint64_t i = 0; i < numStores; i++) *((uint64_t*)(log[i].addr)) = log[i].val;
        for (uint64_t i = 0; i < numStores; i++) PWB(log[i].addr);
        PFENCE();                           // 3rd persistent fence
    }
    ...
    inline uint64_t lookup(const void* addr, uint64_t lval) {
        if (numStores == 0) return lval;
        if (numStores < MAX_ARRAY_LOOKUP) {
            // Lookup in array
            for (unsigned int idx = 0; idx < numStores; idx++) {
...
struct PMetadata {
    static const uint64_t   MAGIC_ID = 0x1337bab7;
...
public:
    struct tmbase : public redologfc::tmbase { };
    WriteSet                               wset {};   // Accessd from the load/store interposing method
    ...
    static std::string className() { return "RedoLog-FC"; }
    ...
    void mapPersistentRegion(const char* filename, uint8_t* regionAddr, const uint64_t regionSize) {
        ...
        if (sizeof(PMetadata) > regionSize/2) {
            ...
            printf("Please reduce some of the settings in RedoLogFC.hpp and try again\n");
            assert(false);
        }
    ...
    inline void endTx() {
        ...
        wset.persistAndFlushLog(&pmd->pwset);
        wset.clear();
        pmd->pwset.clear();
    }

    void recover() {
        // Check if there is an incomplete transaction in the log
        if (pmd->pwset.numStores == 0) return;
        // Apply the log and flush the modifications
        for (int i = 0; i < pmd->pwset.numStores; i++) {
            PWriteSetEntry* e = &pmd->pwset.plog[i];
            *(uint64_t*)e->addr = e->val;
            PWB(e->addr);
        }
        PFENCE();
        pmd->pwset.numStores = 0;
        PWB(&pmd->pwset.numStores);
        PSYNC();
    }
    ...
    template<typename F> static void updateTx(F&& func) { gRedoLog.ns_write_transaction(func); }
    template<typename F> static void readTx(F&& func) { gRedoLog.ns_read_transaction(func); }
    // Sequential durable transactions
    template<typename F> static void updateTxSeq(F&& func) { gRedoLog.beginTx(); func(); gRedoLog.endTx(); }
    ...
    inline static R readTx(F&& func) {
        gRedoLog.ns_read_transaction([&]() {func();});
        return R{};
    }
    template<typename R,class F>
    inline static R updateTx(F&& func) {
        gRedoLog.ns_write_transaction([&]() {func();});
        return R{};
    }
    ...
    template <typename T, typename... Args> static T* tmNew(Args&&... args) {
        ...
        T* ptr = (T*)gRedoLog.esloco.malloc(sizeof(T));
    ...
    static void* tmMalloc(size_t size) {
        ...
        void* obj = gRedoLog.esloco.malloc(size);
        return obj;
    }
    ...
    static void* pmalloc(size_t size) {
        ...
        return gRedoLog.esloco.malloc(size);
    }
...
template<typename T> inline T persist<T>::pload() const {
    ...
    if (valaddr >= (uint8_t*)PM_REGION_BEGIN && valaddr < PM_REGION_END) {
        return (T)gRedoLog.wset.lookup(valaddr, val);
    } else {
        return (T)val;
    }
}
...
template<typename T> inline void persist<T>::pstore(T newVal) {
    const uint8_t* valaddr = (uint8_t*)this;
    assert ((uint64_t)valaddr > 2048);           // Check for null or bad pointers
    if (tl_nested_write_trans != 0 && valaddr >= (uint8_t*)PM_REGION_BEGIN && valaddr < PM_REGION_END) {
        gRedoLog.wset.addOrReplace(this, (uint64_t)newVal);
    } else {
        val = (uint64_t)newVal;
    }
}
...
//
template<typename R, typename F> static R updateTx(F&& func) { return gRedoLog.updateTx<R>(func); }
template<typename R, typename F> static R readTx(F&& func) { return gRedoLog.readTx<R>(func); }
template<typename F> static void updateTx(F&& func) { gRedoLog.updateTx(func); }
template<typename F> static void readTx(F&& func) { gRedoLog.readTx(func); }
template<typename F> static void updateTxSeq(F&& func) { gRedoLog.beginTx(); func(); gRedoLog.endTx(); }
template<typename F> static void readTxSeq(F&& func) { func(); }
template<typename T, typename... Args> T* tmNew(Args&&... args) { return RedoLog::tmNew<T>(std::forward<Args>(args)...); }
template<typename T> void tmDelete(T* obj) { RedoLog::tmDelete<T>(obj); }
inline static void* get_object(int idx) { return RedoLog::get_object(idx); }
inline static void put_object(int idx, void* obj) { RedoLog::put_object(idx, obj); }
inline static void* tmMalloc(size_t size) { return RedoLog::tmMalloc(size); }
inline static void tmFree(void* obj) { RedoLog::tmFree(obj); }
...
#endif /* _REDO_LOG_FLAT_COMBINING_H_ */
