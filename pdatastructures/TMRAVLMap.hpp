 ...
 */
#ifndef _PERSISTENT_TM_RELAXED_AVL_MAP_H_
#define _PERSISTENT_TM_RELAXED_AVL_MAP_H_
 ...
 * If V is larger than size_t, better make it a pointer to the V object.
 *
 * TODO:
...
public:
    ...
    /*
     * Adds a node with a key/value.
     * If a key already exists then replace the value and return false.
     */
    bool put(K key, V val, const int tid=0) {
        bool ret = false;
        TM::updateTx([&] () {
            ...
            Node* n = TM::template tmNew<Node>(key,val);
            n->parent = dst;
            *dstp = n;
            balance(n);
            ret = true;
    ...
    // Same as contains but sets 'val' if the key is found
    V get(K key, const int tid=0) {
        V val {};
        TM::readTx([&] () {
            Node* n = root;
            while (n != nullptr) {
    ...
    bool putAll(K** keys, V** vals, const int size, const int tid=0) {
        for (int i = 0; i < size; i++) put(*keys[i], *vals[i]);
        return true;
    }
...
#endif /* _PERSISTENT_TM_RELAXED_AVL_MAP_H_ */
